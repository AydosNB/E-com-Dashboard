...
const ProductCard = ({ item, categories }) => {
    ...
    return (
        <div ref={ref} className={`${inView? "opasity-100 top-0" : "opacity-0 top-[30px]"} duration-500 relative p-[10px] bg-white shadow-md rounded-sm border-[1px] flex flex-col`}>
            <div className='border-[1px] overflow-hidden rounded-sm relative flex-1'>
                <img className='h-[250px] w-full object-cover' src={item.image} alt={item.title} />
            ...
            <div className='mt-[10px] flex flex-col justify-between flex-1'>
                ...
                <div className='flex justify-between items-end gap-1 flex-1'>
                    <button onClick={() => showModalAlert(item.id, "update")} className='py-[7px] flex justify-center items-center gap-2 rounded-sm px-[10px] w-full bg-blue-500 hover:bg-blue-600 active:scale-95 text-white font-semibold text-[14px]'>
                        <AiFillEdit />
                        <span>
                            Update
                        </span>
                    </button>
                    <button onClick={() => showModalAlert(item.id, "delete")} className='py-[7px] flex justify-center items-center gap-2 rounded-sm px-[10px] w-full bg-red-500 hover:bg-red-600 active:scale-95 text-white font-semibold text-[14px]'>
                        <FaTrash />
                        <span>
                            Delete
                        </span>
