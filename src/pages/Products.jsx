...
const Products = () => {
  ...
  useEffect(() => {
    if (state.modalType === "update") {
      ...
      titleInput.current.value = selectedItem.title,
        descriptionInput.current.value = selectedItem.description,
        ...
        <div className='grid grid-cols-[repeat(auto-fill,minmax(250px,1fr))] gap-2'>
          {
            state.products.map(item => (
              <ProductCard key={item.id} item={item} categories={state.categories} />
            ))
          }
        </div>}
            ...
            <div className='mt-[10px]'>
              <form onSubmit={(e) => handleUpdate(e, state.selectItemId)}>
                ...
                <div className="flex flex-col mt-[10px]">
                  ...
                  <input required ref={imageUrlInput} className="border-[2px] hover:border-blue-100 focus:border-blue-700 outline-none py-[6px] px-[10px] rounded-sm" type="text" id="category-name" placeholder="Enter the image url" />
                </div>
                <div className="flex flex-col mt-[10px]">
